...
async def start_handle(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        f"👋 Salom! <b>{update.message.from_user.full_name}</b>\n\n"
        "Men <b>Chat GPT</b> ning telegram botiman. O'zingizni qiziqtirgan savollarni bering men sizga yordam berishga harakat qilaman 😊", 
...
async def _vision_message_handle_fn(
    ...
    try:
        ...
        placeholder_message = await update.message.reply_text("✏️")
...
async def message_handle(update: Update, context: CallbackContext, message=None, use_new_dialog_timeout=True):
    ...
    async with user_semaphores[user_id]:
        ...
        if (current_model in ["gpt-4-vision-preview", "gpt-4o"]) or (update.message.photo is not None and len(update.message.photo) > 0):
            # Auto-switch to gpt-4o for images if needed
...
async def cancel_handle(update: Update, context: CallbackContext):
    ...
    if user_id in user_tasks:
        ...
        await update.message.reply_text("<i>Bekor qilinadigan hech narsa yo‘q.</i>", parse_mode=ParseMode.HTML)
...
async def show_balance_handle(update: Update, context: CallbackContext):
    ...
    for model_key in sorted(n_used_tokens_dict.keys()):
        ...
        details_text += f"- {model_key}: <b>{n_input_spent_dollars + n_output_spent_dollars:.03f}$</b> / <b>{n_input_tokens + n_output_tokens} token</b>\n"

    # image generation
    image_generation_n_spent_dollars = config.models["info"]["dalle-2"]["price_per_1_image"] * n_generated_images
    if n_generated_images != 0:
        details_text += f"- DALL·E 2 (rasm generatsiyasi): <b>{image_generation_n_spent_dollars:.03f}$</b> / <b>{n_generated_images} generatsiya qilngan rasmlar</b>\n"
    ...
    if n_transcribed_seconds != 0:
        details_text += f"- Whisper (ovozni aniqlash): <b>{voice_recognition_n_spent_dollars:.03f}$</b> / <b>{n_transcribed_seconds:.01f} soniyasiga</b>\n"

    total_n_spent_dollars += voice_recognition_n_spent_dollars


    text = f"Siz <b>{total_n_spent_dollars:.03f}$</b> sarifladingiz\n"
    text += f"<b>{total_n_used_tokens}</b> ta tokendan foydalandingiz\n\n"
    text += details_text
...
async def post_init(application: Application):
    await application.bot.set_my_commands([
        ...
        BotCommand("/new", "Yangi suhbat yaratish"),
        BotCommand("/mode", "Suhbat rejimini tanlash"),
        BotCommand("/retry", "Avvalgi so‘rovga javobni qayta yaratish"),
        BotCommand("/balance", "Balansingizni ko'rish"),
...
def run_bot() -> None:
    ...
    cmds = [
        ...
        ("retry", retry_handle),
        ("new", new_dialog_handle),
        ("cancel", cancel_handle),
        ("mode", show_chat_modes_handle),
        ("settings", settings_handle),
        ("balance", show_balance_handle)
    ]
    for cmd, handler in cmds:
        application.add_handler(CommandHandler(cmd, handler, filters=user_filter))
    ...
    application.add_handler(CallbackQueryHandler(set_settings_handle, pattern=r"^set_settings\|.*"))
